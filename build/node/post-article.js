...
const insertLastPost = () => {
  wp.posts().perPage(1).then((value) => {
    const post = value[0]
    const id = post.id
    const title = post.title.rendered
    const content = post.content.rendered.replace(new RegExp('http://', 'g'), 'https://')
    ...
    database.ref(`/post/${id}`).set({ id, title, content, date, slug, imageId, image, order })
...
function processJson() {
  writeJson(path.join(__dirname, '../jsx/store/articles.json'), jsonPosts.reverse()).then(() => {
